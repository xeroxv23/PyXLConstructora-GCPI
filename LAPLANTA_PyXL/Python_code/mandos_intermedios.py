...
def capturar_mandos_intermedios(clave):
    ...
    porcentaje_mandos_intermedios = '10%'
    ...
    if celda_mandos_intermedios.value == 'lote':
        ...
        #Asignar concepto
        destajo1 = ws.range(fila, columna +7).value = 0.1
        destajo2 = ws.range(fila, columna +11).value = 1
        destajo3 = ws.range(fila, columna +3).value = f"MANDOS INTERMEDIOS  {porcentaje_mandos_intermedios} DE {total_destajo}"
